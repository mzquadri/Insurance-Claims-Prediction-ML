...
#: recorded run was produced with, and until now the results file could not say
#: whether that was still true. Two of the numbers here move in the last place
#: between machines, and one calibration AUC moves further than that because
#: isotonic regression produces ties and a difference in the last bit of a
#: probability can reorder them, so which build produced a file is worth knowing.
RECORDED_LIBRARIES = ("numpy", "pandas", "scikit-learn", "scipy", "joblib")
